...
// їде через OZF_Rpc.RoleRequest -- іншим каналом, під власним іменем мода;
// збираються відповіді там само, де й контактні (OZ_Notice).
//
...
class OZ_PdaPageFaction : OZ_PdaPage
{
    ...
    override void OnResponse(string op, bool ok, string json, string error)
    {
        ...
        m_LastJson = json;
        // Тримаємо КОПІЮ, збудовану скриптом: стан переживає розбір на п'ять
        // секунд і більше, а малюється з нього все -- шапка, склад, тексти
        // підтверджень. Причина довга й лежить в OZ_RoleView.Copy.
        m_St = st.Copy();
        Paint();
    }
