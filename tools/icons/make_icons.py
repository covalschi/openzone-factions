# openzone-factions/tools/icons/make_icons.py
"""Draw the faction glue's icon atlas and write its imageset.
...
`color` tints it. The drawing below is a copy of the `@sprite("tab_faction")`
...
Then convert the PNG with the MCP tool asset_convert -> oz_factions_icons_ca.paa
(the _ca suffix keeps the alpha; _co would drop it). The set is registered in
OpenZone_Factions_PDA/config.cpp (class imageSets); a layout refers to a sprite
as `image0 "set:oz_factions_icons image:tab_faction"`, and the tab rail is told
the same reference by OZ_PdaPageFactory.Sprite().
"""
...
OUT_DIR = ROOT / "OpenZone_Factions_PDA" / "gui" / "imagesets"
SET_NAME = "oz_factions_icons"
TEXTURE = "OpenZone_Factions_PDA/gui/imagesets/oz_factions_icons_ca.paa"
