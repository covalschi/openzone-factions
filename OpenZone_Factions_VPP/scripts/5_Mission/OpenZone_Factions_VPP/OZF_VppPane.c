...
//
// Чіпляється вкладкою до вікна ядра через modded class, рівно як це робить
// панель рації: ядро про фракції не знає й знати не мусить.
//
// НАВІЩО ЦЕ ТУТ, А НЕ В ЯДРІ. Фракційна система виїхала з ядра окремим модом
// 2026-09-01 (рішення власника: у ядрі лишаються служби, а не гра). Панель --
// її обличчя: ростер, ранги, звання, риси, призначення фракцій і пермадес.
...
modded class OZ_VppAdminMenu
{
    // ------------------------------------------------- фракції і гравці
        //
        // Джерело фракцій -- РОСТЕР, а не Factions.json: файла в реєстрі
        // конфігів більше немає (фракції народжує лише бот), а для призначення
        // досить слагів.
        protected ref array<string> m_Factions;
        protected ref array<int> m_FacRowIdx;   // рядок списку -> індекс у m_Factions
        ...
        // Ростер цілком: картці гравця треба все, а не лише ім'я з uid-ом.
        //
        ...
        // Каталоги з реєстру бота, і що зараз під курсором циклерів. FRanks --
        // повні id "duty:sergeant"; циклер показує лише драбину фракції
        // ВИБРАНОГО гравця, тому індекс живе окремо й скидається з вибором.
        ...
        // Пермадес: підтвердження другим натисканням, як і всюди у вкладці
        // (модальні вікна у VPP -- пастка).
    ...
    override void OnCreate(Widget RootW)
    {
        ...
        // ВЛАСНИЙ слухач відповідей, а не гілка в ядровому: ростер і пермадес
        // -- наші операції, і ядро про них більше не знає.
    ...
    override void OnPaneShown(string id)
    {
        ...
        // Циклеру фракцій у панелі спавнів потрібні слаги, а їх дає ростер.
        // Ядро більше не просить його само -- воно й не знає, що він є.
    ...
    void OnRoleAnswer(string op, bool ok, string why)
        {
            ...
            // Ролі їдуть через Discord: перепитуємо ростер трохи згодом, і ще
            // раз пізніше -- проекція вертається не миттєво.
            RefreshSoon();
        }
    ...
    // ---------------------------------------------------------- фракції
        ...
        protected void BuildRoster(OZ_AdminRoster r)
        {
            ...
            // Слаги фракцій і каталоги -- з того самого конверта.
            m_Factions.Clear();
            m_FacLabels.Clear();
            m_FacLimits.Clear();
            m_FacLeaders.Clear();
            ...
            // Вибір переживає оновлення: ростер перечитується сам після кожної
            // операції, і губити від цього виділення -- значить клацати гравця
            // заново після кожної кнопки.
            string keepUid = "";
    ...
    // Список гравців: компактний рядок, виділений позначено стрілкою --
        // підсвітка рядка листбокса непомітна, і це вже коштувало плутанини.
        protected void RepaintRoster()
        {
            ...
            for (int i = 0; i < m_Rows.Count(); i++)
            {
                ...
                line += row.Name;
                // Відсутні теж у списку (ТЗ-4 R-C4.2) -- і позначені: їх можна
    ...
    protected void RebuildFacList()
        {
            ...
            for (int i = 0; i < m_Factions.Count(); i++)
            {
                ...
                // Виділення позначаємо стрілкою: підсвітка рядка листбокса
                // непомітна, і хто виділений -- було не зрозуміло.
                if (i == m_FacPicked)
                    line = "> " + line;
    ...
    // ---------------------------------------------------- редактор фракцій
        ...
        // Форма показує вибрану фракцію; NEW чистить її для нової.
    ...
    // Ціль дій з гравцем: точна адреса uid з ростера. Сервер приймає таку
        // форму лише від адміна -- тезки й самопризначення перестають бути
        // проблемою.
    ...
    override bool OnClick(Widget w, int x, int y, int button)
    {
        ...
        string nm = w.GetName();
                ...
                if (nm == "BtnAssign" || nm == "BtnClearFac" || nm == "BtnLead")
                {
                    ...
                    {
                        // Консоль ставить лідера НАПРЯМУ. leader.transfer тут не
                        // годиться: він -- акт лідера й вимагає, щоб актор сам
                        // тримав пост (зміряно: nobody to hand it over from).
                ...
                if (nm == "BtnRankSet" || nm == "BtnRankClear")
                {
                    ...
                    {
                        // Порожній аргумент -- зняти звання зовсім: так читає його
                        // міст (rank.set без arg знімає всі).
                ...
                if (nm == "BtnWipe")
                {
                    ...
                    // Підтвердження другим натисканням ПО ТОМУ Ж гравцеві: зміна
                    // вибору скидає зброю, інакше підтвердженням для одного стало
                    // б натискання, зроблене для іншого.
                ...
                if (nm == "BtnFacSave")
                {
                    ...
                    // Другим натисканням ПО ТОМУ Ж слагу: зміна форми скидає
                    // курок, інакше підтвердженням для однієї фракції стало б
                    // натискання, зроблене для іншої.
