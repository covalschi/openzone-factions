...
class OZ_Roles
{
    ...
    //
    // ЗВАННЯ, ЯКОГО РЕЄСТР НЕ ЗНАЄ, -- НЕ ЗВАННЯ, і ця перевірка тут не
    // про акуратність (зміряно на стенді 2026-09-06).
    //
    // Поле, значення якого в конверті ПОРОЖНЄ, JsonFileLoader не присвоює
    // взагалі, а ініціалізатор поля (`string FRank = "";`) для об'єкта,
    // якого створив ЗАГРУЗЧИК, не виконується. Одразу після розбору там
    // читається порожньо -- пам'ять свіжа й обнулена, -- і саме таким воно
    // потрапляє в лог і в знімок Seen*. За хвилину купа вже інша, і те саме
    // поле віддає сміття: на екрані фракції стояло "mercenary:3", а на
    // наступному запуску "mercenary:$" -- бо `if (m.FRankId != "")` на
    // такому полі істинне, хоч друкується воно порожнім.
    //
    // Звiряння зi СЛОВНИКОМ прибирає це начисто й водночас відповідає на
    // питання по суті: слаг, якого в реєстрі немає, показувати нема сенсу.
    // Драбина фракції -- OZ_RoleNames, і вона наповнюється тим самим
    // конвертом "roster", що й фракції.
