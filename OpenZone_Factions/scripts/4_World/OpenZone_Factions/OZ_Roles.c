...
class OZ_RoleView
{
    ...
    string Org          = "";
    string Rank         = "";
    // ВНУТРІФРАКЦІЙНЕ звання -- окрема вісь від сталкерського Rank
    // (рішення власника 2026-08-30): живе всередині угруповання, вмирає
    // разом із членством, роздає лідер. Слаг ГОЛИЙ, без префікса фракції --
    // чиє це звання, каже Org поруч.
    string FRank        = "";
    // Видиме ім'я Discord: тільки для адмінських екранів, гравцям не їде.
    string DName        = "";
...
class OZ_Roles
{
    ...
    static void Apply(OZ_RoleView v)
    {
        ...
        if (changed)
        {
            // ЗНІМОК ОНОВЛЮЄТЬСЯ ЗАВЖДИ, А НА ДИСК ЇДЕ ЛИШЕ СВІЙ (ТЗ-1 R1.3
            // з R6.3).
            //
            // Проекція приїжджає й на ВІДСУТНІХ: КПК, який хтось носить у
            // кишені, додає до опиту сесійний uid свого хазяїна
            // (OZ_PdaUidProvider), і міст чесно відповідає про нього.
            //
            // Було два крайніх рішення, і обидва погані. Писати всім --
            // Remember читав файл хазяїна з диска Load-ом, тобто лишав його в
            // кеші сховища до кінця запуску, а звідти він поїхав би й на диск:
            // за одного носія чужого приладу. Не писати нікому -- знімок
            // офлайнового застигав на тому, чим він був у мить свого виходу,
            // хоч свіжа правда вже приїхала й лежить у пам'яті.
            //
            // Тому середина, і вона ж вимога: знімок оновлюється З ПРОЕКЦІЇ
            // ЗАВЖДИ, але для відсутнього -- ЛИШЕ В ПАМ'ЯТІ. Peek дає запис
            // крізь кеш зі строком: файла не заводить, нічого не брудни́ть, і
            // для того, хто в Зоні, віддає рівно той самий об'єкт, що й Load.
            // MarkDirty ставимо тільки йому -- диск міняє власник запису,
            // тобто той, хто зараз тут.
            //
            // ПАМ'ЯТЬ ТУТ НЕ ВІЧНА, і казати про це треба вголос: запис Peek
            // живе своїм строком (п'ять хвилин), після чого читається з диска
            // наново -- разом зі старим знімком. Втрати в цьому немає: свіжу
            // правду про відсутнього однаково тримає кеш проекцій, а Seen()
            // питає його ПЕРШИМ (Of) і до диска доходить, лише коли проекції
            // вже немає.
            //
            // Кеш проекцій (s_By) при цьому лишається головним: саме на ньому
            // тримається доктрина «акаунт називає пристрій» -- без нього чужий
            // КПК не показав би фракції свого хазяїна взагалі.
            bool here = false;
    ...
    //
    // `persist` -- чи має цей знімок доїхати до файла. Для того, кого в Зоні
    // немає, він лишається в пам'яті: див. довгу причину у виклику вище.
    // Peek віддає null, коли файла ще немає взагалі, -- тоді оновлювати нема
    // чого, а заводити файл людині, яка сюди не заходила, тим паче.
    private static void Remember(OZ_RoleView v, bool persist)
    {
        OZ_PlayerData d = OZ_PlayerStore.Peek(v.Uid);
        if (!d)
            return;
    ...
    //
    // ЧИТАЄМО ЧЕРЕЗ Peek, а не Load, і саме тут це важить найбільше: цю
    // функцію питають РІВНО ПРО ВІДСУТНІХ -- контакт із чужого записника,
    // адресат старого повідомлення. Load узяв би кожного такого в кеш
    // сховища до кінця запуску, тобто ріс би від кожного намальованого
    // списку. Той самий об'єкт, який тут читається, оновлює й Remember вище.
    ...
    // Внутріфракційне звання. Порожньо -- без звання, або поза фракцією,
    // або проекція ще не приїхала. Інтерфейс для ЧУЖИХ модів теж: квест
    ...
    // Мітки гравця одним рядком ("medic,mechanic") -- для адмінського
    // ростера. Порожньо -- міток немає або проекція ще не приїхала.
    ...
    // ------------------------------------------- каталоги міток і звань
    //
    // КАТАЛОГИ, а не чиїсь ролі: переліки слагів, які реєстр бота взагалі
    // знає. Потрібні адмінському UI, щоб було з чого вибирати. Живуть тут,
    // бо приїжджають тим самим конвертом роду "roster", що й проекції.
    private static ref array<string> s_TraitIds;
    private static ref array<string> s_RankIds;
    // Внутріфракційні звання, id-шники вигляду "duty:sergeant".
    ...
    // Внутріфракційні звання зберігаємо З ПОРЯДКОМ: без нього «підвищити»
    // не має змісту -- слаги гра знає, а хто з них вищий, каже реєстр.
    ...
    static void RememberFRankIds(array<ref OZ_RoleName> list)
    {
        ...
        for (int i = 0; i < list.Count(); i++)
        {
            ...
            // Порядок могли переставити -- беремо свіжий.
            s_FRankOrder[at] = list[i].Order;
        }
    }

    // Драбина ОДНІЄЇ фракції, знизу вгору: слаги без префікса й підписи
    // поруч. Порожньо -- у фракції звань не завели.
    static void FRankLadder(string faction, out array<string> outIds, out array<string> outNames)
    {
        ...
        for (int i = 0; i < s_FRankIds.Count(); i++)
        {
            ...
            // клієнтові шикованою, інакше «наступне вище» рахуватиме
            // кожен екран сам і по-своєму.
            int at = 0;
...
class OZ_RosterSink : OZ_BridgeSink
{
    override void Deliver(string json)
    {
        ...
        // А СЛАГИ міток і звань -- ще й списками: адмінському UI треба з
        // чого вибирати, а словник вибору не віддає.
