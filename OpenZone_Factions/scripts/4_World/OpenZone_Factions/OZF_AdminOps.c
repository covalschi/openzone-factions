...
class OZ_WipeSink : OZ_BridgeSink
{
    ...
    // ВАЙП ПЕРЕПИСУЄ СКЛАД РОЗМОВ -- отже гасить кеш чату (платформа §4).
    //
    // Це той рідкісний рід, який застарює ЧУЖЕ, і без цього рядка вікно
    // брехні відкрите й вимірюване: сторінка чату оголосила свої
    // v1/chat/list|open|older читальними, ядро тримає відповідь TTL_MS (60 с),
    // а міст на вайпі викреслює uid з members КОЖНОЇ бесіди
    // (openzone-bridge wipePlayer). Конверт роду "wipe" при цьому доїжджає до
    // OZ_BridgeCache.Invalidate, знаходить рід зареєстрованим, дістає
    // ПОРОЖНІЙ список доріг -- і чесно каже «зроблено», не скинувши нічого.
    // Хвилину після пермадесу чужий КПК показував би небіжчика в списку
    // учасників.
    //
